// ── VITARA Chat v11.0 — DB directe, plus de fetch interne ────
...
// Accès DB direct (pas de fetch interne)
async function saveConversationToDB(sessionId: string, state: VitaraState, agentId: string, agentName: string, lang: string, msgs?: any[]) {
  ...
  try {
    ...
    await pool.query(`
      INSERT INTO conversations (
        ...
        pain_scale, language, status, transcript
    ...
    `, [
      ...
      state.full_name?.value || null,
      state.service?.value || null,
      state.practitioner?.value || null,
      state.reason?.value || null,
      state.body_part?.value || null,
      state.accident_type?.value || null,
      state.claim_number?.value || null,
      state.pain_scale?.value || null,
      lang,
      'in_progress',
      JSON.stringify(msgs || []),
    ]);
    ...
    if (state.phone?.value && state.full_name?.value) {
      ...
      await pool.query(`
        ...
        ON CONFLICT (phone) DO UPDATE SET
          full_name     = COALESCE($1, patients.full_name),
          email         = COALESCE($3, patients.email),
          ramq          = COALESCE($4, patients.ramq),
          family_doctor = COALESCE($5, patients.family_doctor),
          updated_at    = NOW()
      `, [
        state.full_name.value,
        phone,
        state.email?.value || null,
        state.ramq?.value || null,
        state.practitioner?.value || null,
      ]);
    }
    await pool.end();
  } catch (e: any) {
    console.error('[VITARA DB]', e.message);
  }
}

// Charger profil patient depuis DB
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Chargement profil DB si téléphone nouveau ───────────────
    ...
    // ── Extraction contextuelle via pending_field ───────────────
    ...
    if (pendingField && (nothingExtracted || !(updates as any)[pendingField])) {
      ...
      if (msg.length >= 2) {
        ...
        if (v) {
          const fu = { [pendingField]: { value: v, status: 'confirmed' as const } };
          newState = applyUpdates(newState, fu as any);
          Object.assign(updates, fu);
        }
      }
    }
    ...
    if (step.type === 'slots') {
      ...
      const speak = language==='ar' ? `إليك 3 مواعيد مع ${prov}. أيها يناسبك؟`
                  : language==='en' ? `Here are 3 available slots with ${prov}. Which works for you?`
    ...
    if (phoneNew && newState.full_name?.value) {
      question = `Rebonjour ${newState.full_name.value.split(' ')[0]} ! J'ai retrouvé votre dossier. ${question}`;
    }
