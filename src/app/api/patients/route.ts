...
// GET /api/patients — liste avec pagination et filtres
export const GET = withAuth(async (req, { user }) => {
  try {
    ...
    let where = `p.clinic_id = $1 AND p.is_active = $2`;
    const params: unknown[] = [user.clinicId, active];
    ...
    if (search) {
      ...
      where += ` AND (
        ...
        p.ramq_number ILIKE $${params.length}
      )`;
    }
    ...
    const sql = `
      SELECT
        ...
        p.phone, p.phone_alt, p.email, p.language,
        p.ramq_number, p.allergies, p.is_active,
...
export const POST = withAuth(async (req, { user }) => {
  try {
    ...
    const existing = await queryOne(
      ...
      [user.clinicId, d.phone]
    );
    if (existing) return apiError('Un patient avec ce numéro existe déjà', 409);

    const patient = await queryOne(
      `INSERT INTO patients (
         id, clinic_id, first_name, last_name, date_of_birth, gender, language,
         phone, phone_alt, email, address, ramq_number, insurance_number,
         insurance_provider, allergies, primary_provider_id, emergency_contact,
      ...
      [
        ...
        d.firstName, d.lastName, d.dateOfBirth, d.gender, d.language,
        d.phone, d.phoneAlt ?? null, d.email || null,
        JSON.stringify(d.address ?? {}),
        d.ramqNumber ?? null, d.insuranceNumber ?? null, d.insuranceProvider ?? null,
        d.allergies,
        d.primaryProviderId ?? null,
